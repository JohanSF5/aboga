...
app.post('/registrarcita', async (req, res) => {
  console.log('Solicitud recibida:', req.body);
const { nombre, correo, telefono, fecha, cita } = req.body;
...
// Ruta para obtener la cantidad de registros en la colección de citas
app.get('/obtener-cantidad-citas', async (req, res) => {
  try {
    const count = await Citas.countDocuments(); // Obtén el número de registros en la colección de citas
    res.json({ count });
  } catch (error) {
    res.status(500).json({ message: 'Error al obtener la cantidad de citas.' });
  }
});
